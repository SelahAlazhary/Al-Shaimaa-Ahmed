...
/**
 * الفصلان الدراسيان مبنيّان في المنصّة.
 * ------------------------------------------------------------------
 * كل منهج مصريّ فصلان، فلا معنى لأن يُطلب من صاحب المنصّة أن يضيفهما
 * يدوياً قبل أن يعمل التسجيل. اللوحة تُبقي إمكانَ إضافة فصول بأسماء
 * أخرى — لكنّها إضافةٌ لا شرط.
 */
 ...
 * الفصل المشترك لكل مرحلة. وبلا فصول مضافة يعود إلى المبنيَّين.
 */
export function termsForStage(
  ...
  const st = s(stage);
  if (!st) return [];
  const own = (terms ?? []).filter((t) => !t.stage || t.stage === st);
  return own.length > 0 ? own : [...BUILT_IN_TERMS];
}
